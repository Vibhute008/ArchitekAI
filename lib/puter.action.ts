...
export const getProjectById = async ({
  ...
  if (!PUTER_WORKER_URL) {
    console.warn("Missing VITE_PUTER_WORKER_URL; falling back to localStorage.");
    try {
      const localProjectsStr = localStorage.getItem("architeckai_local_projects") || "[]";
      const localProjects = JSON.parse(localProjectsStr) as DesignHistoryItem[];
      return localProjects.find((p) => p.id === id) ?? null;
    } catch (error) {
      console.error("Failed to fetch project from localStorage:", error);
      return null;
    }
  }
...
export const getProjects = async () => {
  if (!PUTER_WORKER_URL) {
    console.warn("Missing VITE_PUTER_WORKER_URL; listing from localStorage.");
    try {
      const localProjectsStr = localStorage.getItem("architeckai_local_projects") || "[]";
      const projects = JSON.parse(localProjectsStr) as DesignHistoryItem[];
      return projects.sort((a, b) => (b?.timestamp || 0) - (a?.timestamp || 0));
    } catch (error) {
      console.error("Failed to fetch history from localStorage:", error);
      return [];
    }
  }
...
export const saveProject = async (
  ...
  if (!PUTER_WORKER_URL) {
    console.warn("Missing VITE_PUTER_WORKER_URL; saving to localStorage.");
    try {
      const localProjectsStr = localStorage.getItem("architeckai_local_projects") || "[]";
      const localProjects = JSON.parse(localProjectsStr) as DesignHistoryItem[];
      ...
      if (visibility === "public") {
        payload.sharedBy = payload.sharedBy || "Local User";
        payload.sharedAt = new Date().toISOString();
      }

      const index = localProjects.findIndex((p) => p.id === item.id);
      if (index > -1) {
        localProjects[index] = payload;
      } else {
        localProjects.unshift(payload);
      }

      localStorage.setItem("architeckai_local_projects", JSON.stringify(localProjects));
      return payload;
    } catch (error) {
